...
public class DAO_SanPham {
    ...
    public ArrayList<SanPham> layTatCaSPLoi() {
        ArrayList<SanPham> dsSP = new ArrayList<SanPham>();
        try {
            Connection con = ConnectDB.getInstance().getConnection();
            String sql = "SELECT o.* \n"
                    + "FROM dbo.ChiTietHoanTra AS op \n"
                    + "  JOIN dbo.SanPham AS o\n"
                    + "    ON o.maSP = op.maSP\n"
                    + "WHERE op.lyDoHoanTra = N'Lỗi nhà sản xuất' ";
            Statement statement = con.createStatement();
            ResultSet rs = statement.executeQuery(sql);
            while (rs.next()) {
                String maSP = rs.getString("maSP");
    ...
    public ArrayList<thongKeSPBanChay> thongKeSPBanChayTheoTop(int c, String a, String b) {
        tkSPBC = new ArrayList<thongKeSPBanChay>();
        Connection con = ConnectDB.getInstance().getConnection();
        PreparedStatement stemnt = null;
        int n = 0;
        try {
            ConnectDB.getInstance().connect();
            String sql = "SELECT top(?) Sum(soLuong) as slBan\n"
                    + "      ,maSP\n"
                    + "  FROM [BHTT].[dbo].[ChiTietHoaDon]\n"
                    + "  where   maHD in   ( select maHD\n"
                    + "                    from HoaDon\n"
    ...
    public ArrayList<thongKeSPBanChay> thongKeSPBanChayTheoNam(int a, int b) {
        tkSPBC = new ArrayList<thongKeSPBanChay>();
        Connection con = ConnectDB.getInstance().getConnection();
        PreparedStatement stemnt = null;
        int n = 0;
